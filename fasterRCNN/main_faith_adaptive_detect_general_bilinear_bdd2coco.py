...
from grad_cam_head_faithfulness import GradCAM, apply_gaussian_kernel
...
import logging
logging.basicConfig(filename='/home/jinhanz/cs/xai/logs/250210_fasterrcnn_bilinear_swap.log',
                    filemode='a',
...
def bilinear_rescale(saliency_map, proposals, original_size, preprocessed_size):
    """
    ...
    for i in range(N):
        ...
        x1 = x1.clamp(min=0, max=original_width - 1).floor()
        y1 = y1.clamp(min=0, max=original_height - 1).floor()
        x2 = x2.clamp(min=x1 + 1, max=original_width).ceil()
        y2 = y2.clamp(min=y1 + 1, max=original_height).ceil()
        ...
        resized_feature = F.interpolate(
            saliency_map[i].unsqueeze(0), size=(target_h, target_w), mode='bilinear', align_corners=True)

        # Place the resized features into the corresponding location on the blank original-sized image
        output[i, 0, y1:y2, x1:x2] = resized_feature

    return output

def rescale_maps(saliency_maps_orig_all, proposals_all, h_orig, w_orig, height_preprocessed, width_preprocessed, sel_norm_str):
    masks = []
    masks_sum = torch.zeros((1,1,h_orig,w_orig))
    nObj = 0
    for saliency_map_orig, proposals in zip(saliency_maps_orig_all, proposals_all):
        ...
        if saliency_map_orig.max().item() == 0:
            ...
            if proposals is None:
                ...
                saliency_map = bilinear_rescale(saliency_map_orig, proposals, (h_orig,w_orig), (height_preprocessed, width_preprocessed)).sum(0,keepdim=True)    
...
def main(img_path, label_path, target_layer_group, 
    ...
    _, height_preprocessed, width_preprocessed = image.size()
    ...
    # Saving
    masks, masks_sum = rescale_maps(saliency_maps_orig_all, proposals_all, height, width, height_preprocessed, width_preprocessed, saliency_method.sel_norm_str)
    ...
    # Saving
    masks, masks_sum = rescale_maps(activation_maps_orig_all, proposals_all, height, width, height_preprocessed, width_preprocessed, saliency_method.sel_norm_str)
...
if __name__ == '__main__':
    ...
    if args.object == "vehicle":
        ...
        args.prob = "class"
        args.output_main_dir = "/opt/jinhanz/results/fasterrcnn/bilinear/bdd2coco/xai_saliency_maps_fasterrcnn/fullgradcamraw_vehicle"
        ...
        args.label_path = "/home/jinhanz/cs/data/bdd/orib_veh_id_task0922_mscoco_label"

    elif args.object == "human":
        args.model_path = '/home/jinhanz/cs/xai/models/model_final_721ade.pkl'
        args.method = "fullgradcamraw"
        args.prob = "class"
        args.output_main_dir = "/opt/jinhanz/results/fasterrcnn/bilinear/bdd2coco/xai_saliency_maps_fasterrcnn/fullgradcamraw_human"
        args.coco_labels = "/home/jinhanz/cs/data/mscoco/annotations/COCO_classes2.txt"
        args.img_path = "/home/jinhanz/cs/data/bdd/orib_hum_id_task1009"
        args.label_path = "/home/jinhanz/cs/data/bdd/orib_hum_id_task1009_mscoco_label"

...
path_config.read('./config_v3.ini')
...
def main(img_path, label_path, target_layer_group, 
    ...
    if os.path.exists(os.path.join(args.output_dir.replace('fullgradcamraw','optimization'),f"{img_path.split('/')[-1].split('.')[0]}.json")):
        ...
        # # Find best sigma without tolerance
        # for _, attempt in record['attempts'].items():
        #     if attempt['faithfulness'] > best_faithfulness:
        #         best_sigma = attempt['sigma']
        #         best_faithfulness = attempt['faithfulness']

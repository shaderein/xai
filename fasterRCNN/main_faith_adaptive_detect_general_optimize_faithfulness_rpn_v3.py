...
path_config.read('./config_rpn_v3.ini')
...
def main(img_path, label_path, target_layer_group, 
    ...
    if os.path.exists(os.path.join(args.output_dir.replace('fullgradcamraw','optimization'),f"{img_path.split('/')[-1].split('.')[0]}.json")):
        ...
        sigma = 1
        all_box_length = []
        for box in boxes_rescale_xywh:
            all_box_length.extend(box[2:4])

        sigma_high = min(max(all_box_length), 200)
        ...
        while sigma < sigma_high:
            ...
            masks_attempt, masks_sum_attempt = rescale_and_apply_gaussian(saliency_maps_orig_all, mapped_locs, height, width, sigma, saliency_method.sel_norm_str)
            
            # Calculate faithfulness for both high and mid sigma
            masks_ndarray = masks_sum_attempt.squeeze().detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    if args.object == "COCO":
        ...
        args.prob = "class"
        args.output_main_dir = f"{path_config.get('Paths','result_dir')}/mscoco/rpn_saliency_maps_fasterrcnn/fullgradcamraw"
        ...
        args.method = "fullgradcamraw"
        args.prob = "class"
        args.output_main_dir = f"{path_config.get('Paths','result_dir')}/bdd/rpn_saliency_maps_fasterrcnn/fullgradcamraw_vehicle"
        ...
        args.method = "fullgradcamraw"
        args.prob = "class"
        args.output_main_dir = f"{path_config.get('Paths','result_dir')}/bdd/rpn_saliency_maps_fasterrcnn/fullgradcamraw_human"

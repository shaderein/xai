...
class GradCAM:
    ...
    def forward(self, inputs, index=0):
        ...
        :return:
        """
        output, proposals, proposal_inds = self.net.inference([inputs],do_postprocess=True, output_inds=True, output_proposals=True)
        proposals_boxes = proposals[0].proposal_boxes.tensor.detach()
        ...
        if len(output[0]['instances']) != len(proposal_inds[0]): #DEBUG
            raise Exception("Final instances number doesn't match selected proposals number!")

        for output_score_idx, box_corr, class_id in zip(proposal_inds[0], output[0]['instances'].pred_boxes.tensor, output[0]['instances'].pred_classes):
            if self.class_names[class_id] in self.sel_classes:
            # if self.class_names[class_id] == 'person' or self.class_names[class_id] == 'rider':
                # print(output)
                output_score = proposals[0].objectness_logits[output_score_idx]
                score = torch.sigmoid(output_score)  # DEBUG                                      
                #output[0]['instances'].scores[index]
                ...
                if saliency_map.size()[0] != 1: # more than 1 maps due to different proposals
                    proposals_this_layer = proposals_boxes
                else:
                    proposals_this_layer = None

                # Note: proposals have the same coordinates as the preprocessed input image (with fixed height 800)
                proposals_all.append(proposals_this_layer)
                ## Rescale based on receptive field
                receptive_field, jump, start = calculate_receptive_field(self.layer_name, self.layers, proposals_this_layer)

...
target_layer_group_dict = {'F1': ['model_17_act', 'model_21_act', 'model_25_act'],                    #F1
                           'F2': ['model_14_act', 'model_19_act', 'model_23_act'],                    #F2
                           'F3': ['model_13_act', 'model_17_act', 'model_21_act'],                    #F3
                           'F4': ['model_10_act', 'model_14_act', 'model_19_act'],                    #F4
                           'F5': ['model_9_act', 'model_13_act', 'model_17_act'],                     #F5
                           'F6': ['model_9_act', 'model_10_act', 'model_14_act'],                     #F6
                           'F7': ['model_9_act', 'model_9_act', 'model_13_act'],                      #F7
                           'F8': ['model_9_act', 'model_9_act', 'model_10_act'],                      #F8
                           'F9': ['model_9_act', 'model_9_act', 'model_9_act'],                       #F9
                           'F10': ['model_8_cv2_act', 'model_8_cv2_act', 'model_8_cv2_act'],           #F10
                           'F11': ['model_7_act', 'model_7_act', 'model_7_act'],                       #F11
                           'F12': ['model_6_act', 'model_6_act', 'model_6_act'],                       #F12
                           'F13': ['model_5_act', 'model_5_act', 'model_5_act'],                       #F13
                           'F14': ['model_4_act', 'model_4_act', 'model_4_act'],                       #F14
                           'F15': ['model_3_act', 'model_3_act', 'model_3_act'],                       #F15
                           'F16': ['model_2_act', 'model_2_act', 'model_2_act'],                       #F16
                           'F17': ['model_1_act', 'model_1_act', 'model_1_act']}                       #F17
...
# target_layer_group_list = [['model_14_act', 'model_19_act', 'model_23_act']]                       #F1
# target_layer_group_name_list = ['F1']

# target_layer_group_list = [['model_8_cv2_act', 'model_8_cv2_act', 'model_8_cv2_act']]                       #F10
# target_layer_group_name_list = ['F10']

# target_layer_group_list = [['model_1_act', 'model_1_act', 'model_1_act']]                       #F9
# target_layer_group_name_list = ['F17']

# input_main_dir = 'orib_veh_id_task_previous'   #Veh_id_img orib_veh_id_task_previous orib_veh_id_task0922
# input_main_dir_label = 'orib_veh_id_task_previous_label'   #Veh_id_label orib_veh_id_task_previous_label orib_veh_id_task0922_label
# output_main_dir = 'orib_veh_id_task_previous_yolov5s300epoch_results_negGrad'

input_main_dir = 'orib_veh_id_task_previous'   #Veh_id_img orib_veh_id_task_previous orib_veh_id_task0922
input_main_dir_label = 'orib_veh_id_task_previous_label'   #Veh_id_label orib_veh_id_task_previous_label orib_veh_id_task0922_label
output_main_dir = 'orib_veh_id_task_previous_yolov5s300epoch_results_HAGXAI'

sel_method = 'fullgradcam'  # gradcam, gradcampp, fullgradcam, fullgradcamraw, saveRawGradAct, saveRawAllAct, DRISE, HAGXAI
...
"""
perturb_level
    target: delete/insert area size up to the total area of target BBs
    image: delete/insert area size up to the whole image
perturb_ratio 
    (default 100): delete/insert 100% of the target/image
"""
def main(img_path, label_path, model, saliency_method, img_num, perturb_level='target',perturb_ratio=100):
    gc.collect()
    torch.cuda.empty_cache()
    ...
    tRuntime = round(time.time() - tic, 4)
    print("total time:", tRuntime)
    ...
    if sel_faith == 'nofaith':
        ...
        preds_deletion, preds_insertation, _, imgs_deletion, imgs_insertion = ut.compute_faith(model, img, masks_ndarray, label_data_corr_xywh, class_names_sel,perturb_level,perturb_ratio)
        # compress gif
        ...
        imageio.mimsave(f'{os.path.join(args.output_dir,img_name+".deletion")}.gif', imgs_deletion_new, fps=24)
        imageio.mimsave(f'{os.path.join(args.output_dir,img_name+".insertion")}.gif', imgs_insertion_new, fps=24)
        # Saving
        scipy.io.savemat(output_path + '.mat', mdict={'masks_ndarray': masks_ndarray,
                                                      'boxes_pred_xyxy': boxes_rescale_xyxy,
                                                      'boxes_pred_xywh': boxes_rescale_xywh,
...
if __name__ == '__main__':
    ...
    target_layer_group_name_list = ['F1','F10']
    target_layer_group_list = [target_layer_group_dict[i] for i in target_layer_group_name_list]


    for i, (target_layer_group, target_layer_group_name) in enumerate(zip(target_layer_group_list, target_layer_group_name_list)):
        
        # for perturb_level in ['image','target']:
        #     for perturb_ratio in [25,50,75,100]:
        for perturb_level in ['target']:
            for perturb_ratio in [100]:


                sub_dir_name = perturb_level + '_' + str(perturb_ratio) + '_' + target_layer_group_name + '_' + sel_method + '_' + sel_object + '_' + sel_nms + '_' + sel_prob + '_' + sel_faith + '_' + sel_norm + '_' + sel_model_str
                ...
                if os.path.isdir(args.img_path):
                    img_list = os.listdir(args.img_path)#[:3]
                    label_list = os.listdir(args.label_path)#[:3]
                    print(img_list)
                    for item_img, item_label in zip(img_list, label_list):

                        t = main(os.path.join(args.img_path, item_img), os.path.join(args.label_path, item_label), model, saliency_method, item_img[:-4],
                                 perturb_level,perturb_ratio)
                        t_sum = t_sum+t

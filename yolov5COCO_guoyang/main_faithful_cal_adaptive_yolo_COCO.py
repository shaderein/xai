...
def main(img_path, label_path, model, saliency_method, img_num, class_names_sel):
    gc.collect()
    torch.cuda.empty_cache()
    ...
    for i, (target_layer_group_name, target_layer_group) in enumerate(target_layer_group_dict.items()):
        ...
        if os.path.isdir(args.img_path):
            ...
            for item_img, item_label in zip(img_list, label_list):
                ...
                main(os.path.join(args.img_path, item_img), os.path.join(args.label_path, item_label), model, saliency_method, item_img[:-4],[class_name])

...
logging.basicConfig(filename='/home/jinhanz/cs/xai/logs/241127_fasterrcnn_rpn_faithfulness_all.log', 
                    filemode='a',
...
for model in ['fasterrcnn']: #'yolov5s'
    for dataset in ['mscoco','vehicle','human']:
        for is_act in ['']:
            for upscale_type in ['bilinear','optimize_faithfulness']:
                root_dir = f"/opt/jinhanz/results/{upscale_type}/"
                if dataset != 'mscoco':
                    xai_path = os.path.join(root_dir,'bdd',f"rpn_saliency_maps_{model}")
                else:
                    xai_path = os.path.join(root_dir,dataset,f"rpn_saliency_maps_{model}")

...
logging.basicConfig(filename='/home/jinhanz/cs/xai/logs/250212_yolov5s_faithfulness_v2.5_10steps_bdd2coco.log', 
                    filemode='a',
                    format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',
                    datefmt='%H:%M:%S',
                    level=logging.INFO)

root_dir = "/opt/jinhanz/results/yolov5s/optimize_faithfulness_finer_v2.5_10steps/"
    ...
    for dataset in ['human']:
        for is_act in ['']:
            for upscale_type in ['bilinear']:
                if dataset != 'mscoco':
                    xai_path = os.path.join(root_dir,'bdd2coco',f"xai_saliency_maps_{model}")
                else:

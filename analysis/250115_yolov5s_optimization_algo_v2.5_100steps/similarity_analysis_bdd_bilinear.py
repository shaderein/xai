...
logging.basicConfig(filename='/home/jinhanz/cs/xai/logs/250115_yolov5s_correlation_process_bdd.log', 
                    level=logging.INFO,
...
for object in ['vehicle','human']: #,
    ...
    for rescale_method in ['bilinear']: # ,
        for is_act in ["xai_saliency"]: #,
            ...
            xai_saliency_path = {
                "FullGradCAM":f'/opt/jinhanz/results/yolov5s/{rescale_method}/bdd/{is_act}_maps_yolov5s/fullgradcamraw_{object}',
                "ODAM":f'/opt/jinhanz/results/yolov5s/{rescale_method}/bdd/{is_act}_maps_yolov5s/odam_{object}',
            }
            ...
            for type, path_by_type in xai_saliency_path.items():
                ...
                for dir in tqdm.tqdm(os.listdir(path_by_type)):
                    ...
                    for file in os.listdir(os.path.join(path_by_type,dir)):
                        ...
                        img_idx = file.replace('-res.mat','').replace('-res.mat','').replace('-res.png.mat','').replace('-res.jpg.mat','')
            ...
            for method, func in similarity_methods.items():
                ...
                save_dir = f'/home/jinhanz/cs/xai/results/bdd/250115_{rescale_method}_{is_act}_maps_yolov5s'

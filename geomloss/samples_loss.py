...
class SamplesLoss(Module):
    ...
    def forward(self, *args):
        ...
        l_x, α, x, l_y, β, y, cost_value = self.process_args(*args)
        ...
        if backend in [
            ...
            if B == 1:
                ...
                warnings.warn(
                    "'online' and 'multiscale' backends do not support batchsize > 1. "
                    + "Using 'tensorized' instead: beware of memory overflows!"
                )
                backend = "tensorized"

        if (
            B == 0 and backend == "tensorized"
        ):  # tensorized routines work on batched tensors
    ...
    def process_args(self, *args):
        if len(args) == 7:
            ...
            return l_x, α, x, l_y, β, y, None
        elif len(args) == 5:
            α, x, β, y, cost_value = args
            return None, α, x, None, β, y, cost_value
        elif len(args) == 4:
            α, x, β, y = args

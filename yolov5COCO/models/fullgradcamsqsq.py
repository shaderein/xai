...
class YOLOV5FullGradCAMsqsq:

    def __init__(self, model, layer_names, sel_prob_str, sel_norm_str, img_size=(640, 640)):
        self.model = model
        self.gradients = dict()
    ...
    def forward(self, input_img, class_idx=True):
        ...
        with torch.autograd.set_detect_anomaly(True):
            for pred_logit, logit, bbox, cls, cls_name, obj_prob, class_prob, classHead in zip(preds_logits[0], logits[0], preds[0][0], preds[1][0], preds[2][0], preds[3][0], class_probs, classHead_output):
                if cls_name == 'car' or cls_name == 'truck' or cls_name == 'bus':
                    if class_idx:
                        if self.sel_prob_str == 'obj' or self.sel_prob_str == 'xy' or self.sel_prob_str == 'wh':
                            score = pred_logit
                        elif self.sel_prob_str == 'classall':
                            score = logit.sum()
                        else:
                            score = logit[cls]
                    ...
                    saliency_map[torch.isnan(saliency_map)] = 0
                    nObj = nObj + 1
                    if nObj == 1:
        ...
        if nObj != 0:
            ...
            # del self, saliency_map_sum, saliency_map, preds_logits, preds, pred_logit, obj_prob, logits, logit, input_img, class_probs, class_prob, weights, gradients, activations, alpha, score
        gc.collect()
        torch.cuda.empty_cache()

        return saliency_maps, pred_list, class_prob_list, head_num_list

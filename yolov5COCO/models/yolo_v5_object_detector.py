...
class YOLOV5TorchObjectDetector(nn.Module):
    ...
    def forward(self, img):
        prediction, logits, full_logits = self.model(img, augment=False)
        pred_conf_logits = torch.cat((torch.flatten(full_logits[0], 1,3), torch.flatten(full_logits[1], 1,3), torch.flatten(full_logits[2], 1,3)), 1)
        classHead = torch.cat((torch.flatten(torch.zeros(full_logits[0].shape).mean(dim=4)+0, 1,3), torch.flatten(torch.zeros(full_logits[1].shape).mean(dim=4)+1, 1,3), torch.flatten(torch.zeros(full_logits[2].shape).mean(dim=4)+2, 1,3)), 1)
        pred_conf_logits = pred_conf_logits[:,:,0:5]
